...
TEST ( QuaternionRotateVectorNoRotation ) {
	SimpleMath::GL::Quaternion q_norot = SimpleMath::GL::Quaternion ();

	Vector3f vec (1.f, 2.f, 3.f);
	Vector3f rotated = q_norot.rotate (vec);

	CHECK_ARRAY_CLOSE (vec.data(), rotated.data(), 3, TEST_PREC);
}
...
TEST ( QuaternionFromGLMatrix ) {
	Vector3f axis(Vector3f(23.f, 0.1f, 0.33f).normalize());

	SimpleMath::GL::Quaternion q_rot = SimpleMath::GL::Quaternion::fromGLRotate(23.f, axis[0], axis[1], axis[2]);
	SimpleMath::GL::Quaternion q_from_matrix = SimpleMath::GL::Quaternion::fromGLMatrix (q_rot.toGLMatrix());

	CHECK_ARRAY_CLOSE (q_rot.data(), q_from_matrix.data(), 4, TEST_PREC);
}
